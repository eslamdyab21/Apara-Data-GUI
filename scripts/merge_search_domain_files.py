...
def DirctoryPathToMergeSmallFiles(current_directory, small_file_max_size_kb, merged_file_max_size_mb):
    ...
    if len(df_multi_list[0]) > 0:
        ...
        for df_list in df_multi_list:
            print('following files will be merged in: '+'"domain_search_merged_file_ '+str(list_num+1) + '.csv"')
            s = 'following files will be merged in: '+'"domain_search_merged_file_ '+str(list_num+1) + '.csv"'
            yield s
            ...
            if len(df_list) >0:
                ...
                big_df.to_csv(final_directory + '/' + 'domain_search_merged_file_1_'+str(list_num+1) + '.csv', index=None, header=True)
                list_num = list_num + 1
